...
class ElectricityScreen extends React.Component {
	...
	state = {
		counter_id: '', created_by: '', profile: {}, lastValue: 0, newValue: 0,
		store_id: '', type_id: ''
	}
	...
	handleAddElectricity = async () => {
		...
		Alert.alert('Send Value', 'Are you Sure',
			[
	...
	async componentDidMount() {
		...
		await GET(GET_USER_STORE_BY_USER_ID + "/" + this.state.profile.id,
			{}, {}
		).then(res => {
			if (res.status == 200) {
		...
		await POST(GET_COUNTER_BY_TYPE_STORE_ID, {}, {}, {
		...
		await GET(GET_COUNTER_TIME_BY_TIME + "/" + this.state.counter_id,
			{}, {}
		).then(res => {
			if (res.status == 200) {
					...
					<View style={{ flex: 3, marginLeft: 15, marginTop: 7 }}>
						<Text style={{ fontSize: 20 }}>
							<VectorIcon
								...
								color="#00365d"
							/>{" "}
							{this.state.newValue - this.state.lastValue}/{this.state.newValue} KWh New/All
            	</Text>
						<Slider
							minimumTrackTintColor='#13a9d6'
							step={1}
							onValueChange={newValue => this.setState({ newValue: newValue })}
					...
					<View style={{ flex: 3, marginLeft: 15, marginTop: 7 }}>
						<Text style={{ fontSize: 20 }}>
							<VectorIcon
								...
								color="#00365d"
							/>{" "}
							{this.state.newValue - this.state.lastValue}/{this.state.newValue} m3 New/All
            	</Text>
						<Slider
							minimumTrackTintColor='#13a9d6'
							step={1}
